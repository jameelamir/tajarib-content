...
function tryFetchYouTubeTranscript(slug, videoUrl) {
  return new Promise((resolve) => {
    ...
    proc.on("close", () => {
      // Look for downloaded json3 subtitle files
      let files;
      try {
        files = fs.readdirSync(epDir).filter(f => f.startsWith("yt-subs") && f.endsWith(".json3"));
      } catch {
        files = [];
      }
      ...
      // Prefer Arabic, fall back to English, then whatever is available
      ...
      try {
        ...
        const outPath = path.join(epDir, "transcript.json");
        fs.writeFileSync(outPath, JSON.stringify(transcript, null, 2), "utf-8");

        io.emit("log", { slug, text: `✅ YouTube transcript saved: ${transcript.segment_count} segments, ${transcript.word_count} words\n` });

        // Clean up json3 files
...
function convertYouTubeJson3(raw, slug, sourceUrl, language) {
  ...
  return {
    ...
    word_count: wordsAll.length,
    segment_count: segments.length,
    full_text: fullTextParts.join(" "),
    segments,
    words: wordsAll,
    api_provider: "youtube"
  };
}
...
async function handler(req, res) {
  ...
  // ── Assets API (overlay files: sponsor.mov, logo.mov, lower-third, etc.) ───
  ...
  if (req.method === "GET" && url.pathname === "/api/assets/browse") {
    const typeFilter = url.searchParams.get("type"); // e.g. "video", "image", or null for all
        ...
        for (const name of fs.readdirSync(dirPath)) {
          ...
          try {
            ...
            results.push({
              ...
              isSymlink,
              symlinkTarget: isSymlink ? fs.readlinkSync(fullPath) : null
            });
    ...
    // Also scan shared assets directory if configured
  ...
  // Serve asset files for browser preview (images, first frame of videos)
  ...
  if (req.method === "GET" && assetFileMatch) {
    ...
    // For video files, extract a single frame as JPEG for preview
    if ([".mov", ".mp4", ".avi", ".mkv"].includes(ext)) {
      ...
      if (!fs.existsSync(thumbPath) || fs.statSync(thumbPath).mtimeMs < srcStat.mtimeMs) {
        try {
          ...
          res.writeHead(500); res.end("Thumbnail failed"); return;
        }
      }
  ...
  if (req.method === "POST" && url.pathname === "/api/link-asset") {
    ...
    req.on("end", () => {
      try {
        ...
        // Determine destination name: use provided assetName or derive from source
    ...
    req.on("end", () => {
      try {
        ...
        proc.on("close", async (code) => {
          ...
          // Try to get YouTube transcript first, fall back to Whisper
          const gotYouTubeTranscript = await tryFetchYouTubeTranscript(slug, videoUrl);
          if (gotYouTubeTranscript) {
            io.emit("log", { slug, text: "📝 Using YouTube transcript — skipping Whisper.\n" });
            await handlePostTranscription(slug);
            io.emit("status-update", {});
          } else {
